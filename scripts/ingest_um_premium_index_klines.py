...
def main() -> int:
    ...
    ap.add_argument("--intervals", required=True, help="comma-separated, e.g. 1h,4h")
    ap.add_argument("--start", required=True, help="ISO date or datetime, e.g. 2024-01-01 or 2024-01-01T00:00:00Z")
    ...
    basket = load_basket(manifest, args.basket)
    intervals = [s.strip() for s in args.intervals.split(",") if s.strip()]

    client = BinanceDataArchiveClient()
    arch = UmPremiumIndexKlinesArchive(data_root=data_root, client=client)
    report = arch.ingest_range(
        symbols=basket,
        intervals=intervals,
    ...
    out = runs / f"{ts}_um_premium_index_klines_report.json"
